...
# paths declarations
daily_csv_path = "coinbase_btc_eur_historic_data.csv"
hourly_csv_path = "coinbase_btc_eur_historic_data_hourly.csv"
modelh5_filename = "model.h5"
current_directory = os.path.dirname(os.path.abspath(__file__))
hourly_directory = os.path.join(current_directory, 'hourly_data')
daily_directory = os.path.join(current_directory, 'daily_data')


class RecurrentNeuralNetworkHourly(Resource):
    def get(self):
        return {'RNN': 'GET Page'}
    ...
    def post(self):
        ...
        if(hourly == False):
            ...
            csv_path = send_from_directory(daily_directory, daily_csv_path)
            normalizer_path = os.path.join(daily_directory, "normalizer.json")
            modelh5_path = os.path.join(daily_directory, "model.h5")
        else:
            epochs = 270
            directory = hourly_directory
            csv_path = send_from_directory(hourly_directory, hourly_csv_path)
            normalizer_path = os.path.join(hourly_directory, "normalizer.json")
            modelh5_path = os.path.join(hourly_directory, "model.h5")
        print("Loaded")
